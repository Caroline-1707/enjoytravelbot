...
from keyboards.inline.hotels import get_count_photos, get_count_hotel
from handlers.custom_handlers.operations.functions import get_search_results
import datetime
    ...
    if stop_iter:
        try:
            ...
            bot.send_message(message.from_user.id,
                             f'Выбери пункт назначения из списка:', reply_markup=city_markup(answer))
...
def callback_inline(call: CallbackQuery):
    ...
    if action == "DAY":
        bot.send_message(
            ...
            text=f"Выбранная дата: {my_date.strftime('%d.%m.%Y')}"
        )
        with bot.retrieve_data(call.message.chat.id, call.message.chat.id) as hotels_data:
    ...
    if action == "DAY":
        ...
        with bot.retrieve_data(call.message.chat.id, call.message.chat.id) as hotels_data:
            if datetime.date(int(year), int(month), int(day)) >= hotels_data['check_in']:
                ...
                bot.send_message(call.message.chat.id, 'Вывести отелей:',
                                 reply_markup=get_count_hotel())
                bot.set_state(call.message.chat.id, HotelPriceState.count_hotel)
            else:
                bot.send_message(call.message.chat.id, 'Дата выезда не может быть меньше даты заезда.')
                create_calendar(call.message, hotels_data['check_in'])
...
def get_count_hotels(call) -> None:
    """ Коллбэк для получения количества выводимых отелей """
    with bot.retrieve_data(call.message.chat.id, call.message.chat.id) as hotels_data:
        hotels_data['hotels_count'] = int(call.data)
        bot.send_message(call.message.chat.id, 'Отлично, нужны фото выбранных отелей? Количество фото:',
...
def count_photos(call) -> None:
    ...
    bot.edit_message_text(chat_id=call.message.chat.id,
                          message_id=call.message.id,
                          text=f'Показать фото для каждого отеля: {call.data}')
    bot.send_message(call.message.chat.id, 'Ищу подходящие варианты...')
    bot.set_state(call.message.chat.id, HotelPriceState.info, call.message.chat.id)
    with bot.retrieve_data(call.message.chat.id, call.message.chat.id) as hotels_data:
        command = hotels_data['command']
        sort_order = 'PRICE_LOW_TO_HIGH' if command == '/highprice' or command == '/lowprice' else 'DISTANCE'
